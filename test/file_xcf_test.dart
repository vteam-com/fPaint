...
void main() {
  testWidgets('Simply Run the app', (WidgetTester tester) async {
    try {
      ...
      print('XCF Header Information:');
      print('Magic: ${xfcFile.magic}');
      print('Version: ${xfcFile.versionInfo}');
      print('Width: ${xfcFile.width}');
      print('Height: ${xfcFile.height}');
      print('Base Type: ${xfcFile.baseTypeString}');
      print('\nLayers (${xfcFile.layers.length}):');
      for (var i = 0; i < xfcFile.layers.length; i++) {
        print('${i + 1}. ${xfcFile.layers[i]}');
      }
    } catch (e) {
      print('Error reading XCF file: $e');
    }
  });
}
