...
void main() {
  group('AppSvgIcon', () {
    testWidgets('creates SvgPicture with explicit color', (WidgetTester tester) async {
      const Widget widget = AppSvgIcon(icon: AppIcon.brush, color: Colors.red);
      expect(widget, isA<Widget>());
    });

    testWidgets('isSelected true produces blue icon', (WidgetTester tester) async {
      const Widget widget = AppSvgIcon(icon: AppIcon.brush, isSelected: true);
      expect(widget, isA<Widget>());
    });

    testWidgets('isSelected false produces white icon', (WidgetTester tester) async {
      const Widget widget = AppSvgIcon(icon: AppIcon.brush, isSelected: false);
      expect(widget, isA<Widget>());
    });

    testWidgets('preserves the provided color alpha when tinting svg assets', (WidgetTester tester) async {
